...
프로젝트 목록 다음, 연락처 앞에 날짜순 기록 섹션이 하나 있다 (`#devlog`).
편집기에서 **데브로그** 그룹을 열어 **[+ 글 추가]** 로 늘리면 된다 — 날짜 · 제목 ·
...
별도 URL은 없고 한 섹션 안에 리스트로 쌓인다 — 새 글이 늘어도 사이트 구조나
디자인은 안 바뀐다.

## 미니게임 「너구리」

맨 아래 연락처 밑에 **[잠깐 쉬어가기 — 너구리]** 버튼이 있다. 누르면 창이 열린다.
...
(`content.json` 의 `site.miniGame` 이 `false` 가 되고, 게임 코드가 `index.html` 에서
통째로 빠진다). 버튼 문구는 `site.gameLabel`, 옆 설명은 `site.gameNote` 로 바꾼다.
